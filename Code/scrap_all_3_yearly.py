...
print(f"Todays's Date : {current_date}")
    ...
    for month in range(1, 13):
        ...
        for day in range(1, 32):
            ...
            if date==current_date:
                for i in range(0,len(all_dates)-1,365):
                    df = yf.download('GC=F', all_dates[i], all_dates[i+365],interval='1d')

                    # saving the dataframe
                    print(f"{all_dates[i]}:{all_dates[i+1]}")
                    #df.to_csv('../Database/gold_rate_max.csv', mode='a', index=False, header=False)
                    df.to_csv(f'../Database/yearly/{all_dates[i]}.csv', mode='a')
